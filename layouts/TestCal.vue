...
const dropPost = (event, date, hour = null) => {
  ...
  if (dropDate < today) {
    return; // Block drops on past dates
  }
  const dateStr = date.toISOString().split("T")[0];
...
const confirmTime = () => {
  if (draggedPost.value) {
    const postIndex = posts.value.findIndex(
      (p) => p.id === draggedPost.value.id
    );
    if (postIndex !== -1) {
      posts.value[postIndex].time =
...
// Get posts for a specific date and hour, ordered by time
const getPostsForDateTimeOrdered = (date, hour) => {
  ...
  // Sort posts by time within the hour
