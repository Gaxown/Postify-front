<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="!selectedTeamId" class="text-center py-12">
        <div class="text-gray-500 mb-4">
          Please select a team to view profiles
        </div>
      </div>
    ...
    <CreateFormModal ref="createModalRef" type="profile" />
...
const initializeFromUrl = async () => {
  ...
  const teamIdFromParams = route.params.id
...
const fetchProfiles = async () => {
  ...
  try {
    ...
    const response = (await $fetch(
      `${apiBase}/teams/${selectedTeamId.value}/profiles`,
      {
        method: "GET",
        headers,
      }
    ...
    profiles.value = response.map((profile) => ({
      ...
      avatar: profile.avatar
        ? `${apiBase.replace("/api", "")}/storage/${profile.avatar}`
        : "",
